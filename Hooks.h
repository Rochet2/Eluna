...
{
    ENV_NONE = 0,
    ENV_MAP = 2,                    // For current map only
    ENV_WORLD = 4,                  // For world state only
    ENV_BOTH = ENV_MAP | ENV_WORLD, // For world and map
};
...
enum BindingType
{
    ...
    BINDTYPE_ITEM,
    BINDTYPE_MAP,
    BINDTYPE_PLAYERGOSSIP,
};

class Hooks
{
public:
    typedef std::unordered_map<std::string, ElunaEnvironments> HookContainer;

    static const HookContainer eventsServer;
    template<BindingType T>
    struct TypeSpecific
    {
        static const HookContainer events;
    };

    static ElunaEnvironments GetEventEnv(HookContainer const & hook_container, std::string const & event_id);
};
